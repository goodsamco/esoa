...
/* ==========================================================================
   10. PLACE THIS ALWAYS AT LAST - ADMINISTRATIVE REMOTE HOT-RELOAD & CACHE-BUSTER MATRIX
   ========================================================================== */
if (userId) {
    let initialSignalSkipped = false;
    
    // Listen directly to the user's account configuration node for the administrative heartbeat
    onSnapshot(doc(db, "accounts", userId), (snapshot) => {
        if (snapshot.exists()) {
            const userData = snapshot.data();
            
            if (userData && userData.refreshSignal) {
                // Safeguard: Prevents an immediate recursive refresh loop on initial page mount
                if (!initialSignalSkipped) {
                    initialSignalSkipped = true;
                    return;
                }
                
                // Generate a unique epoch timestamp string for cache interception
                const versionMarker = new Date().getTime();
                
                console.log("[System Sync] Admin remote refresh signal detected. Flushing local asset caches...");

                // Step 1: Hot-swap and force upgrade all linked CSS styles instantly
                document.querySelectorAll('link[rel="stylesheet"]').forEach(link => {
                    try {
                        const styleUrl = new URL(link.href, window.location.href);
                        styleUrl.searchParams.set('v', versionMarker);
                        link.href = styleUrl.toString();
                    } catch (err) {
                        // Soft structural fallback for cross-origin style bindings
                        link.href = link.href.split('?')[0] + '?v=' + versionMarker;
                    }
                });

                // Step 2: Enforce hard execution context reload to fetch the latest JS builds
                const targetCleanUrl = window.location.origin + window.location.pathname;
                
                setTimeout(() => {
                    window.location.href = `${targetCleanUrl}?v=${versionMarker}${window.location.hash}`;
                }, 150); // Minimal processing window to let visual layout modifications settle cleanly
            }
        }
    });
}
